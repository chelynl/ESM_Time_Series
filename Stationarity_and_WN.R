...
for(i in 1:10){
  White.LB_diff[i] <- Box.test(diff_data, lag = i, type = "Lj", fitdf = 1)$p.value
}
...
# Lags 1-3 are not significant but the following lags are.
...
# We need to take care of dependencies by taking trend and seasonality into consideration.
