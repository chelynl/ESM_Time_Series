# Check the stationarity of the average monthly Ozone levels including any potential trend
# and/or random walks; the analysts recommend using the Augmented Dickey-Fuller tests
# up to lag 2 tests for the results, however, you are welcome to suggest other techniques
...
# fitdf tells how many autocorrelation terms to fit (sum of p and q), we set to 0 because we have not fit any AR or MA terms
...
barplot(White.LB_diff, 
        main = "Ljung-Box Test for Differenced Series P-values", 
        ylab = "Probabilities", 
        xlab = "Lags", 
        ylim = c(0, 0.2),
        names.arg = seq(0,9))
...
# Lags 0-4 are not significant (stationary) but the following lags are (still some correlation structure).
# We want all lags to be insignificant and fail to reject OVERALL --> WN... but we don't have that
# This concludes that autocorrelation exists in the differenced series --> Reject null.
